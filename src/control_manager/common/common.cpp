...
std::optional<unsigned int> idxInVector(const std::string& str,
...
bool validateTrackerCommand(
    const rclcpp::Node::SharedPtr& node,
    const std::optional<mrs_msgs::msg::TrackerCommand>& msg,
    const std::string& var_name) {
  if (!msg) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "the optional variable '%s' is not set!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->position.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->position.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->position.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->position.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->position.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->position.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->velocity.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->velocity.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->velocity.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->velocity.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->velocity.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->velocity.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->acceleration.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->acceleration.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->acceleration.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->acceleration.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->acceleration.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->acceleration.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->jerk.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->jerk.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->jerk.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->jerk.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->jerk.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->jerk.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->snap.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->snap.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->snap.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->snap.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->snap.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->snap.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->attitude_rate.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->attitude_rate.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->attitude_rate.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->attitude_rate.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->attitude_rate.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->attitude_rate.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->heading)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->heading'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->heading_rate)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->heading_rate'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->heading_acceleration)) {
    RCLCPP_ERROR_THROTTLE(
        ...
        "NaN detected in variable '%s->heading_acceleration'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->heading_jerk)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->heading_jerk'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg->throttle)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s->throttle'!!!",
                          var_name.c_str());
    return false;
  }
...
bool validateOdometry(const rclcpp::Node::SharedPtr& node,
  ...
  if (!std::isfinite(msg.pose.pose.position.x)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.pose.pose.position.x'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.pose.position.y)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.pose.pose.position.y'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.pose.position.z)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.pose.pose.position.z'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.pose.orientation.x)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.pose.pose.orientation.x'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.pose.orientation.y)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.pose.pose.orientation.y'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.pose.orientation.z)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.pose.pose.orientation.z'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.pose.orientation.w)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.pose.pose.orientation.w'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.twist.twist.linear.x)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.twist.twist.linear.x'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.twist.twist.linear.y)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.twist.twist.linear.y'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.twist.twist.linear.z)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.twist.twist.linear.z'!!!",
        var_name.c_str());
    return false;
  }
...
bool validateVelocityReference(const rclcpp::Node::SharedPtr& node,
  ...
  if (!std::isfinite(msg.altitude)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.altitude'!!!",
                          var_name.c_str());
    return false;
  }
...
bool validateReference(const rclcpp::Node::SharedPtr& node,
...
bool validateUavState(const rclcpp::Node::SharedPtr& node,
  ...
  if (!std::isfinite(msg.pose.position.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.pose.position.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.position.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.pose.position.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.position.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.pose.position.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.orientation.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.pose.orientation.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.orientation.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.pose.orientation.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.orientation.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.pose.orientation.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.pose.orientation.w)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.pose.orientation.w'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.velocity.linear.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.velocity.linear.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.velocity.linear.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.velocity.linear.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.velocity.linear.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.velocity.linear.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.velocity.angular.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.velocity.angular.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.velocity.angular.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.velocity.angular.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.velocity.angular.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.velocity.angular.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration.linear.x)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration.linear.x'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration.linear.y)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration.linear.y'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration.linear.z)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration.linear.z'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration.angular.x)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration.angular.x'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration.angular.y)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration.angular.y'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration.angular.z)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration.angular.z'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration_disturbance.angular.x)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration_disturbance.angular.x'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration_disturbance.angular.y)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration_disturbance.angular.y'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration_disturbance.angular.z)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration_disturbance.angular.z'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration_disturbance.linear.x)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration_disturbance.linear.x'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration_disturbance.linear.y)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration_disturbance.linear.y'!!!",
        var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.acceleration_disturbance.linear.z)) {
    RCLCPP_ERROR_THROTTLE(
        node->get_logger(), *node->get_clock(), 1000,
        "NaN detected in variable '%s.acceleration_disturbance.linear.z'!!!",
        var_name.c_str());
    return false;
  }
...
std::optional<DetailedModelParams_t> loadDetailedUavModelParams(
    const rclcpp::Node::SharedPtr& node, const std::string& platform_config,
  ...
  if (!enabled) {
    RCLCPP_INFO(node->get_logger(),
                "detailed UAV model params were not provided");
    return {};
  }
  ...
  param_loader.loadParam("model_params/propulsion/force_constant",
                         force_constant);
  param_loader.loadParam("model_params/propulsion/torque_constant",
                         torque_constant);
  ...
  param_loader.loadParam("model_params/inertia_matrix/enabled",
                         inertia_enabled);

  param_loader.loadMatrixDynamic("model_params/propulsion/allocation_matrix",
                                 allocation_matrix, Eigen::Matrix4d::Identity(),
                                 4, -1);

  if (!param_loader.loadedSuccessfully()) {
    RCLCPP_WARN(
        node->get_logger(),
        "detailed UAV model params not loaded, missing some parameters. This "
        "will not permit operations when ACTUATORS or CONTROL_GROUP control "
        "outputs would be possible.");
    return {};
  } else {
    RCLCPP_INFO(node->get_logger(),
                "detailed UAV model params successfully loaded.");
  }
  ...
  if (inertia_enabled) {
    bool inertia_loaded = param_loader.loadMatrixStatic(
        "model_params/inertia_matrix/matrix", inertia_matrix,
        Eigen::Matrix3d::Identity());

    if (inertia_loaded) {
      model_params.inertia = inertia_matrix;
      RCLCPP_INFO(node->get_logger(),
                  "inertia matrix loaded from config file:");
      RCLCPP_INFO_STREAM(node->get_logger(), model_params.inertia);

    } else {
      RCLCPP_ERROR(node->get_logger(),
                   "inertia matrix should be loaded from yaml, but was not "
                   "loaded sucessfully!");
      rclcpp::shutdown();
      exit(1);
    }
    ...
    RCLCPP_INFO(node->get_logger(),
                "computing inertia it from the UAV detailed model params");

    // create the inertia matrix
    model_params.inertia = Eigen::Matrix3d::Zero();
    model_params.inertia(0, 0) =
        mass * (3.0 * arm_length * arm_length + body_height * body_height) /
        12.0;
    model_params.inertia(1, 1) =
        mass * (3.0 * arm_length * arm_length + body_height * body_height) /
        12.0;
  ...
  model_params.force_torque_mixer.row(2) *=
      torque_constant * (3.0 * prop_radius) * force_constant;
  ...
  Eigen::MatrixXd alloc_tmp = model_params.force_torque_mixer.transpose() *
                              (model_params.force_torque_mixer *
                               model_params.force_torque_mixer.transpose())
                                  .inverse();
...
std::optional<double> extractThrottle(
    const Controller::ControlOutput& control_output) {
  if (!control_output.control_output) {
    return {};
  }

  return std::visit(HwApiCmdExtractThrottleVisitor(),
...
bool validateControlOutput(const rclcpp::Node::SharedPtr& node,
                           const Controller::ControlOutput& control_output,
                           const ControlOutputModalities_t& output_modalities,
                           const std::string& var_name) {
  if (!control_output.control_output) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "the optional variable '%s' is not set!!!",
  ...
  std::variant<ControlOutputModalities_t> output_modalities_var{
      output_modalities};
  std::variant<std::string> var_name_var{var_name};
  std::variant<rclcpp::Node::SharedPtr> node_var{node};

  return std::visit(HwApiValidateVisitor(), node_var,
                    control_output.control_output.value(),
                    output_modalities_var, var_name_var);
}
...
bool validateHwApiActuatorCmd(const rclcpp::Node::SharedPtr& node,
                              const mrs_msgs::msg::HwApiActuatorCmd& msg,
                              const std::string& var_name) {
  for (size_t i = 0; i < msg.motors.size(); i++) {
    if (!std::isfinite(msg.motors[i])) {
      RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                            "NaN detected in variable '%s.motors[%d]'!!!",
                            var_name.c_str(), int(i));
      return false;
    }
  }
...
bool validateHwApiControlGroupCmd(
  ...
  if (!std::isfinite(msg.pitch)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.pitch'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.yaw)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.yaw'!!!",
                          var_name.c_str());
    return false;
  }
...
bool validateHwApiAttitudeCmd(const rclcpp::Node::SharedPtr& node,
  ...
  if (!std::isfinite(msg.orientation.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.orientation.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.orientation.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.orientation.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.orientation.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.orientation.z'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.orientation.w)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.orientation.w'!!!",
                          var_name.c_str());
    return false;
  }
...
bool validateHwApiAttitudeRateCmd(
    const rclcpp::Node::SharedPtr& node,
  ...
  if (!std::isfinite(msg.body_rate.x)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.body_rate.x'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.body_rate.y)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.body_rate.y'!!!",
                          var_name.c_str());
    return false;
  }
  ...
  if (!std::isfinite(msg.body_rate.z)) {
    RCLCPP_ERROR_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                          "NaN detected in variable '%s.body_rate.z'!!!",
                          var_name.c_str());
    return false;
  }
...
bool validateHwApiAccelerationHdgRateCmd(
    const rclcpp::Node::SharedPtr& node,
...
bool validateHwApiAccelerationHdgCmd(
    const rclcpp::Node::SharedPtr& node,
...
bool validateHwApiVelocityHdgRateCmd(
    const rclcpp::Node::SharedPtr& node,
...
bool validateHwApiVelocityHdgCmd(const rclcpp::Node::SharedPtr& node,
...
Controller::HwApiOutputVariant initializeDefaultOutput(
    const rclcpp::Node::SharedPtr& node,
    const ControlOutputModalities_t& possible_outputs,
    const mrs_msgs::msg::UavState& uav_state, const double& min_throttle,
    const double& n_motors) {
  ...
  switch (lowest_output) {
    ...
    case TRAJECTORY: {
      output = mrs_msgs::msg::HwApiTrajectoryCmd();
      break;
    }
  }
  ...
  std::visit(HwApiInitializeVisitor(), node_var, output, uav_state_var,
             min_throttle_var, n_motors_var);
...
void initializeHwApiCmd(const rclcpp::Node::SharedPtr& node,
                        mrs_msgs::msg::HwApiActuatorCmd& msg,
...
void initializeHwApiCmd(const rclcpp::Node::SharedPtr& node,
                        mrs_msgs::msg::HwApiControlGroupCmd& msg,
...
void initializeHwApiCmd(const rclcpp::Node::SharedPtr& node,
                        mrs_msgs::msg::HwApiAttitudeRateCmd& msg,
                        const double& min_throttle) {
  msg.stamp = node->get_clock()->now();
...
void initializeHwApiCmd(const rclcpp::Node::SharedPtr& node,
                        mrs_msgs::msg::HwApiAttitudeCmd& msg,
...
void initializeHwApiCmd(const rclcpp::Node::SharedPtr& node,
                        mrs_msgs::msg::HwApiAccelerationHdgRateCmd& msg,
                        const mrs_msgs::msg::UavState& uav_state) {
...
void initializeHwApiCmd(const rclcpp::Node::SharedPtr& node,
                        mrs_msgs::msg::HwApiAccelerationHdgCmd& msg,
                        const mrs_msgs::msg::UavState& uav_state) {
  msg.header.frame_id = uav_state.header.frame_id;
  msg.header.stamp = node->get_clock()->now();
...
void initializeHwApiCmd(const rclcpp::Node::SharedPtr& node,
                        mrs_msgs::msg::HwApiVelocityHdgRateCmd& msg,
                        const mrs_msgs::msg::UavState& uav_state) {
  msg.header.frame_id = uav_state.header.frame_id;
  msg.header.stamp = node->get_clock()->now();
...
void initializeHwApiCmd(const rclcpp::Node::SharedPtr& node,
                        mrs_msgs::msg::HwApiPositionCmd& msg,
                        const mrs_msgs::msg::UavState& uav_state) {
  msg.header.frame_id = uav_state.header.frame_id;
  msg.header.stamp = node->get_clock()->now();
...
/* initializeHwApiCmd(mrs_msgs::msg::HwApiTrajectoryCmd& msg) //{ */

void initializeHwApiCmd(const rclcpp::Node::SharedPtr& node,
                        mrs_msgs::msg::HwApiTrajectoryCmd& msg,
                        const mrs_msgs::msg::UavState& uav_state) {
  msg.header.frame_id = uav_state.header.frame_id;
  msg.header.stamp = node->get_clock()->now();
