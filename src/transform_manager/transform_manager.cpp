...
class TransformManager : public mrs_lib::Node {
 ...
 private:
  ...
  void callbackAltitudeAmsl(
      const mrs_msgs::msg::HwApiAltitude::ConstSharedPtr msg);

  mrs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped>
      sh_hw_api_orientation_;
  void callbackHwApiOrientation(
  ...
  std::optional<geometry_msgs::msg::Pose> transformRtkToFcu(
      const geometry_msgs::msg::PoseStamped& pose_in) const;

  void publishFcuUntiltedTf(
      const geometry_msgs::msg::QuaternionStamped::ConstSharedPtr msg);
...
TransformManager::TransformManager(rclcpp::NodeOptions options)
...
void TransformManager::initialize() {
  ...
  RCLCPP_INFO(node_->get_logger(), "[%s]: initializing",
              getPrintName().c_str());
  ...
  if (Support::toLowercase(world_origin_units_) == "utm") {
    RCLCPP_INFO(node_->get_logger(), "[%s]: Loading world origin in UTM units.",
                getPrintName().c_str());
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[%s]: Loading world origin in LatLon units.",
                getPrintName().c_str());
    ...
    RCLCPP_INFO(node_->get_logger(), "[%s]: Converted to UTM x: %f, y: %f.",
                getPrintName().c_str(), world_origin_x, world_origin_y);

  } else {
    RCLCPP_ERROR(node_->get_logger(),
                 "[%s]: mrs_uav_managers/world_origin/units must be "
                 "(\"UTM\"|\"LATLON\"). Got '%s'",
                 getPrintName().c_str(), world_origin_units_.c_str());
    rclcpp::shutdown();
  }
  ...
  if (!is_origin_param_ok) {
    RCLCPP_ERROR(node_->get_logger(),
                 "[%s]: Could not load all mandatory parameters from world "
                 "file. Please check your world file.",
                 getPrintName().c_str());
    rclcpp::shutdown();
  }
  ...
  param_loader.loadParam(yaml_prefix + "local_origin_tf/parent",
                         local_origin_parent_frame_id);
  ns_local_origin_parent_frame_id_ =
      ch_->uav_name + "/" + local_origin_parent_frame_id;

  std::string local_origin_child_frame_id;
  param_loader.loadParam(yaml_prefix + "local_origin_tf/child",
                         local_origin_child_frame_id);
  ns_local_origin_child_frame_id_ =
      ch_->uav_name + "/" + local_origin_child_frame_id;

  param_loader.loadParam(yaml_prefix + "local_origin_tf/enabled",
                         publish_local_origin_tf_);
  /*//}*/

  /*//{ load stable_origin parameters */
  std::string stable_origin_parent_frame_id;
  param_loader.loadParam(yaml_prefix + "stable_origin_tf/parent",
                         stable_origin_parent_frame_id);
  ns_stable_origin_parent_frame_id_ =
      ch_->uav_name + "/" + stable_origin_parent_frame_id;

  std::string stable_origin_child_frame_id;
  param_loader.loadParam(yaml_prefix + "stable_origin_tf/child",
                         stable_origin_child_frame_id);
  ns_stable_origin_child_frame_id_ =
      ch_->uav_name + "/" + stable_origin_child_frame_id;

  param_loader.loadParam(yaml_prefix + "stable_origin_tf/enabled",
                         publish_stable_origin_tf_);
  /*//}*/

  /*//{ load fixed_origin parameters */
  std::string fixed_origin_parent_frame_id;
  param_loader.loadParam(yaml_prefix + "fixed_origin_tf/parent",
                         fixed_origin_parent_frame_id);
  ns_fixed_origin_parent_frame_id_ =
      ch_->uav_name + "/" + fixed_origin_parent_frame_id;

  std::string fixed_origin_child_frame_id;
  param_loader.loadParam(yaml_prefix + "fixed_origin_tf/child",
                         fixed_origin_child_frame_id);
  ns_fixed_origin_child_frame_id_ =
      ch_->uav_name + "/" + fixed_origin_child_frame_id;

  param_loader.loadParam(yaml_prefix + "fixed_origin_tf/enabled",
                         publish_fixed_origin_tf_);
  /*//}*/
  ...
  param_loader.loadParam(yaml_prefix + "fcu_untilted_tf/child",
                         fcu_untilted_frame_id);
  ch_->frames.fcu_untilted = fcu_untilted_frame_id;
  ch_->frames.ns_fcu_untilted = ch_->uav_name + "/" + fcu_untilted_frame_id;

  param_loader.loadParam(yaml_prefix + "fcu_untilted_tf/enabled",
                         publish_fcu_untilted_tf_);
  /*//}*/

  /*//{ load amsl_origin parameters*/
  std::string amsl_parent_frame_id, amsl_child_frame_id;
  param_loader.loadParam(yaml_prefix + "altitude_amsl_tf/enabled",
                         publish_amsl_origin_tf_);
  param_loader.loadParam(yaml_prefix + "altitude_amsl_tf/parent",
                         amsl_parent_frame_id);
  param_loader.loadParam(yaml_prefix + "altitude_amsl_tf/child",
                         amsl_child_frame_id);
  ...
  param_loader.loadParam(yaml_prefix + "rtk_antenna/frame_id",
                         ch_->frames.rtk_antenna);
  ch_->frames.ns_rtk_antenna = ch_->uav_name + "/" + ch_->frames.rtk_antenna;

  param_loader.loadParam("mrs_uav_managers/estimation_manager/state_estimators",
                         estimator_names_);
  param_loader.loadParam(yaml_prefix + "tf_sources", tf_source_names_);

  param_loader.loadParam(yaml_prefix + "utm_source_priority",
                         utm_source_priority_list_);

  for (auto utm_source : utm_source_priority_list_) {
    if (Support::isStringInVector(utm_source, estimator_names_)) {
      RCLCPP_INFO(node_->get_logger(),
                  "[%s]: the source for utm_origin and world origin is: %s",
                  getPrintName().c_str(), utm_source.c_str());
  ...
  for (size_t i = 0; i < tf_source_names_.size(); i++) {
    ...
    RCLCPP_INFO(node_->get_logger(), "[%s]: loading tf source: %s",
                getPrintName().c_str(), tf_source_name.c_str());

    auto source_param_loader = std::make_shared<mrs_lib::ParamLoader>(
        node_, "TransformManager/" + tf_source_name);
    ...
    tf_sources_.push_back(
        std::make_unique<TfSource>(tf_source_name, node_, source_param_loader,
                                   broadcaster_, ch_, is_utm_source));
  }
  ...
  for (int i = 0; i < int(estimator_names_.size()); i++) {
    ...
    RCLCPP_INFO(node_->get_logger(), "[%s]: loading tf source of estimator: %s",
                getPrintName().c_str(), estimator_name.c_str());

    auto estimator_param_loader = std::make_shared<mrs_lib::ParamLoader>(
        node_, "TransformManager/" + estimator_name);
    ...
    tf_sources_.push_back(std::make_unique<TfSource>(
        estimator_name, node_, estimator_param_loader, broadcaster_, ch_,
        is_utm_source));
  }

  // initialize mapping_origin tf
  bool mapping_origin_tf_enabled;
  param_loader.loadParam(yaml_prefix + "mapping_origin_tf/enabled",
                         mapping_origin_tf_enabled, false);

  if (mapping_origin_tf_enabled) {
    auto mapping_param_loader = std::make_shared<mrs_lib::ParamLoader>(
        node_, "TransformManager/mapping_origin_tf");
    ...
    tf_mapping_origin_ = std::make_unique<TfMappingOrigin>(
        node_, mapping_param_loader, broadcaster_, ch_);
  }
  ...
  sh_uav_state_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::UavState>(
      shopts, "~/uav_state_in", &TransformManager::callbackUavState, this);

  sh_height_agl_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::Float64Stamped>(
      shopts, "~/height_agl_in", &TransformManager::callbackHeightAgl, this);

  sh_altitude_amsl_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::HwApiAltitude>(
      shopts, "~/altitude_amsl_in", &TransformManager::callbackAltitudeAmsl,
      this);

  sh_hw_api_orientation_ =
      mrs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped>(
          shopts, "~/orientation_in",
          &TransformManager::callbackHwApiOrientation, this);

  if (utm_source_name_ == "rtk" || utm_source_name_ == "rtk_garmin") {
    sh_rtk_gps_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::RtkGps>(
        shopts, "~/rtk_gps_in", &TransformManager::callbackRtkGps, this);
  } else {
    sh_gnss_ = mrs_lib::SubscriberHandler<sensor_msgs::msg::NavSatFix>(
        shopts, "~/gnss_in", &TransformManager::callbackGnss, this);
  }
  /*//}*/
  ...
  srvs_set_world_origin_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::ReferenceStampedSrv>(
          ...
          std::bind(&TransformManager::callbackSetWorldOrigin, this,
  ...
  if (!param_loader.loadedSuccessfully()) {
    RCLCPP_ERROR(
        node_->get_logger(),
        "[%s]: Could not load all non-optional parameters. Shutting down.",
        getPrintName().c_str());
    rclcpp::shutdown();
  }
  ...
  for (int i = 0; i < 10; i++) {
    auto res_tf_rtk = ch_->transformer->getTransform(
        ch_->frames.ns_rtk_antenna, ch_->frames.ns_fcu, clock_->now());
    if (res_tf_rtk) {
      RCLCPP_INFO(node_->get_logger(), "[%s] got tf from FCU to RTK antenna",
                  getPrintName().c_str());
      got_rtk_antenna_tf = true;
      break;
    }
    RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "[%s] %s tf from FCU to RTK antenna",
                         getPrintName().c_str(),
  ...
  if (!got_rtk_antenna_tf) {
    RCLCPP_ERROR(node_->get_logger(),
                 "[%s]: The transform from FCU to RTK antenna is not defined. "
                 "Please provide static tf from %s to %s.",
                 getPrintName().c_str(), ch_->frames.ns_fcu.c_str(),
                 ch_->frames.ns_rtk_antenna.c_str());
    rclcpp::shutdown();
  }
...
void TransformManager::callbackUavState(
  ...
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, "TransformManager::publishFcuUntilted",
  ...
  // publish static tf from fixed_origin to local_origin based on the first
  // message
  ...
  if (publish_stable_origin_tf_) {
    ...
    pose.pose = msg->pose;
    if (pose.pose.orientation.w == 0 && pose.pose.orientation.z == 0 &&
        pose.pose.orientation.y == 0 && pose.pose.orientation.x == 0) {
      RCLCPP_WARN_ONCE(node_->get_logger(),
                       "[%s]: Uninitialized quaternion detected during "
                       "publishing stable_origin tf of %s. Setting w=1",
    ...
    if (res) {
      ...
      if (Support::noNans(tf_msg)) {
        ...
        RCLCPP_WARN_THROTTLE(
            ...
            "[%s]: NaN detected in transform from %s to %s. Not publishing tf.",
            getPrintName().c_str(), tf_msg.header.frame_id.c_str(),
            tf_msg.child_frame_id.c_str());
      }
      RCLCPP_INFO_ONCE(node_->get_logger(),
                       "[%s]: Broadcasting transform from parent frame: %s to "
                       "child frame: %s",
                       getPrintName().c_str(), tf_msg.header.frame_id.c_str(),
                       tf_msg.child_frame_id.c_str());
    } else {
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "[%s]: Could not transform pose to %s. Not "
                            "publishing stable_origin transform.",
  ...
  if (publish_fixed_origin_tf_) {
    ...
    if (msg->header.frame_id != last_frame_id_) {
      RCLCPP_WARN(node_->get_logger(),
                  "[%s]: Detected estimator change from %s to %s. Updating "
                  "offset for fixed origin.",
                  getPrintName().c_str(), last_frame_id_.c_str(),
    ...
    RCLCPP_INFO_ONCE(
        node_->get_logger(),
        "[%s]: Broadcasting transform from parent frame: %s to child frame: %s",
        getPrintName().c_str(), tf_msg.header.frame_id.c_str(),
        tf_msg.child_frame_id.c_str());
    /*//}*/
  }
  ...
  if (msg->header.frame_id != last_frame_id_) {
    const std::string last_estimator_name =
        Support::frameIdToEstimatorName(last_frame_id_);
    const std::string current_estimator_name =
        Support::frameIdToEstimatorName(msg->header.frame_id);

    RCLCPP_INFO(node_->get_logger(),
                "[%s]: Detected estimator switch: %s -> %s",
                getPrintName().c_str(), last_estimator_name.c_str(),
    ...
    for (size_t i = 0; i < tf_sources_.size(); i++) {
      // first check if tf source can publish utm origin and is not the
      // switched-from estimator
      if (tf_sources_.at(i)->getIsUtmBased() &&
          tf_sources_.at(i)->getName() != last_estimator_name) {
        valid_utm_source_found = true;
        potential_utm_source_index = i;

        // check if switched-to estimator is utm_based, if so, use it
        if (tf_sources_.at(i)->getIsUtmBased() &&
            tf_sources_.at(i)->getName() == current_estimator_name) {
          potential_utm_source_index = i;
          break;
        }
      }
    ...
    // if we found a valid utm source, use it, otherwise stay with the
    // switched-from estimator
    if (valid_utm_source_found) {
      ...
      for (size_t i = 0; i < tf_sources_.size(); i++) {
        if (tf_sources_.at(i)->getIsUtmSource()) {
          ...
          RCLCPP_INFO(node_->get_logger(),
                      "[%s]: setting is_utm_source of estimator %s to false",
                      getPrintName().c_str(), last_estimator_name.c_str());
        }
      }
      ...
      RCLCPP_INFO(node_->get_logger(),
                  "[%s]: setting is_utm_source of estimator %s to true",
                  getPrintName().c_str(), current_estimator_name.c_str());
    }
  }
  /*//}*/
...
void TransformManager::callbackHeightAgl(
    const mrs_msgs::msg::Float64Stamped::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }
  ...
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, "TransformManager::callbackHeightAgl",
                          ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void TransformManager::callbackAltitudeAmsl(
    [[maybe_unused]] const mrs_msgs::msg::HwApiAltitude::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }

  // Currently not used. Not clear what this message from hw_api should be for.
  // Transform manager publishes the AMSL altitude from RTK or GNSS messages.
}
...
void TransformManager::publishAmslTf(const double altitude,
                                     const rclcpp::Time& stamp) {
  if (!is_initialized_) {
    return;
  }

  // Currently not used. Not clear what this message should be for. Altitude
  // data is taken eiter from RTK or GNSS.
  geometry_msgs::msg::TransformStamped tf_msg;
...
void TransformManager::callbackHwApiOrientation(
    const geometry_msgs::msg::QuaternionStamped::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }
...
void TransformManager::callbackGnss(
  ...
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, "TransformManager::callbackGnss",
                          ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  if (!std::isfinite(out_x)) {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[%s]: NaN detected in UTM variable \"out_x\"!!!",
                          getPrintName().c_str());
    return;
  }
  ...
  if (!std::isfinite(out_y)) {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[%s]: NaN detected in UTM variable \"out_y\"!!!",
                          getPrintName().c_str());
    return;
  }
  ...
  RCLCPP_INFO(node_->get_logger(),
              "[%s]: utm_origin position calculated as: x: %.2f, y: %.2f, z: "
              "%.2f from GNSS",
              getPrintName().c_str(), utm_origin.x, utm_origin.y, utm_origin.z);
...
void TransformManager::callbackRtkGps(
  ...
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, "TransformManager::callbackRtkGps",
                          ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  if (!std::isfinite(msg->gps.latitude)) {
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "[%s] NaN detected in RTK variable \"msg->latitude\"!!!",
        getPrintName().c_str());
    return;
  }
  ...
  if (!std::isfinite(msg->gps.longitude)) {
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "[%s] NaN detected in RTK variable \"msg->longitude\"!!!",
        getPrintName().c_str());
    return;
  }
  ...
  if (!std::isfinite(msg->gps.altitude)) {
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "[%s] NaN detected in RTK variable \"msg->altitude\"!!!",
        getPrintName().c_str());
    return;
  }

  if (msg->fix_type.fix_type != mrs_msgs::msg::RtkFixType::RTK_FIX) {
    RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000, "[%s] %s RTK FIX",
                         getPrintName().c_str(),
                         Support::waiting_for_string.c_str());
    return;
  }
  ...
  mrs_lib::UTM(msg->gps.latitude, msg->gps.longitude, &rtk_pos.pose.position.x,
               &rtk_pos.pose.position.y);
...
bool TransformManager::callbackSetWorldOrigin(
  ...
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, "TransformManager::callbackSetWorldOrigin",
                          ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void TransformManager::publishFcuUntiltedTf(
    const geometry_msgs::msg::QuaternionStamped::ConstSharedPtr msg) {
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, "TransformManager::publishFcuUntilted",
                          ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  try {
    ...
    RCLCPP_ERROR(node_->get_logger(),
                 "[%s]: Exception caught during getting heading",
                 getPrintName().c_str());
    return;
  }
  scope_timer.checkpoint("heading");

  const Eigen::Matrix3d odom_pixhawk_R =
      mrs_lib::AttitudeConverter(msg->quaternion);
  const Eigen::Matrix3d undo_heading_R = mrs_lib::AttitudeConverter(
      Eigen::AngleAxis(-heading, Eigen::Vector3d(0, 0, 1)));

  const tf2::Quaternion q =
      mrs_lib::AttitudeConverter(undo_heading_R * odom_pixhawk_R);
  ...
  if (Support::noNans(tf)) {
    ...
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[%s]: NaN encountered in fcu_untilted tf",
                          getPrintName().c_str());
  }
...
void TransformManager::publishLocalTf() {
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, getPrintName() + "::publishLocalTf",
                          ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  tf_msg.transform.translation =
      Support::pointToVector3(pose_first_.pose.position);
  ...
  if (Support::noNans(tf_msg)) {
    try {
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "[%s]: exception caught while publishing tf from %s to %s",
                   getPrintName().c_str(), tf_msg.header.frame_id.c_str(),
                   tf_msg.child_frame_id.c_str());
    }
...
std::optional<geometry_msgs::msg::Pose> TransformManager::transformRtkToFcu(
    const geometry_msgs::msg::PoseStamped& pose_in) const {
  geometry_msgs::msg::PoseStamped pose_tmp = pose_in;

  // inject current orientation into rtk pose
  auto res1 = ch_->transformer->getTransform(ch_->frames.ns_fcu_untilted,
                                             ch_->frames.ns_fcu, clock_->now());
  if (res1) {
    pose_tmp.pose.orientation = res1.value().transform.rotation;
  } else {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[%s]: Could not obtain transform from %s to %s.",
                          getPrintName().c_str(),
  ...
  auto res2 =
      ch_->transformer->transformSingle(utm_in_antenna, ch_->frames.ns_fcu);

  if (res2) {
    utm_in_fcu = res2.value();
  } else {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[%s]: Could not transform RTK pose from %s to %s.",
                          getPrintName().c_str(),
                          utm_in_antenna.header.frame_id.c_str(),
                          ch_->frames.ns_fcu.c_str());
    return {};
  }
  ...
  geometry_msgs::msg::Pose fcu_in_utm =
      Support::poseFromTf2(tf_fcu_to_utm.inverse());
...
std::string TransformManager::getName() const { return name_; }

//}

/* getPrintName() //{ */

std::string TransformManager::getPrintName() const { return nodelet_name_; }
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    mrs_uav_managers::transform_manager::TransformManager)
