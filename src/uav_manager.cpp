...
class UavManager : public mrs_lib::Node {
 ...
 public:
  ...
  bool callbackLand(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackLandHome(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackLandThere(
  ...
  bool callbackMidairActivation(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  rclcpp::Time midair_activation_started_;

  bool callbackMinHeightCheck(
...
UavManager::UavManager(rclcpp::NodeOptions options)
    : mrs_lib::Node("uav_manager", options) {
  node_ = this_node_ptr();
  ...
  timer_hw_api_capabilities_ = node_->create_wall_timer(
      ...
      std::bind(&UavManager::timerHwApiCapabilities, this));
}
...
void UavManager::initialize() {
  ...
  param_loader.loadParam(yaml_prefix + "takeoff/after_takeoff/tracker",
                         _after_takeoff_tracker_name_);
  param_loader.loadParam(yaml_prefix + "takeoff/after_takeoff/controller",
                         _after_takeoff_controller_name_);
  param_loader.loadParam(yaml_prefix + "takeoff/during_takeoff/controller",
                         _takeoff_controller_name_);
  param_loader.loadParam(yaml_prefix + "takeoff/during_takeoff/tracker",
                         _takeoff_tracker_name_);
  param_loader.loadParam(yaml_prefix + "takeoff/takeoff_height",
                         _takeoff_height_);

  if (_takeoff_height_ < 0.5 || _takeoff_height_ > 10.0) {
    RCLCPP_ERROR(
        node_->get_logger(),
        "the takeoff height (%.2f m) has to be between 0.5 and 10 meters",
        _takeoff_height_);
    rclcpp::shutdown();
    exit(1);
  }
  ...
  param_loader.loadParam(yaml_prefix + "landing/landing_tracker",
                         _landing_tracker_name_);
  param_loader.loadParam(yaml_prefix + "landing/landing_controller",
                         _landing_controller_name_);
  param_loader.loadParam(yaml_prefix + "landing/landing_cutoff_mass_factor",
                         _landing_cutoff_mass_factor_);
  param_loader.loadParam(yaml_prefix + "landing/landing_cutoff_timeout",
                         _landing_cutoff_mass_timeout_);
  param_loader.loadParam(yaml_prefix + "landing/disarm", _landing_disarm_);
  param_loader.loadParam(yaml_prefix + "landing/descend_height",
                         _landing_descend_height_);
  param_loader.loadParam(yaml_prefix + "landing/tracking_tolerance/translation",
                         _landing_tracking_tolerance_translation_);
  param_loader.loadParam(yaml_prefix + "landing/tracking_tolerance/heading",
                         _landing_tracking_tolerance_heading_);

  param_loader.loadParam(yaml_prefix + "midair_activation/rate",
                         _midair_activation_timer_rate_);
  param_loader.loadParam(
      yaml_prefix + "midair_activation/during_activation/controller",
      _midair_activation_during_controller_);
  param_loader.loadParam(
      yaml_prefix + "midair_activation/during_activation/tracker",
      _midair_activation_during_tracker_);
  param_loader.loadParam(
      yaml_prefix + "midair_activation/after_activation/controller",
      _midair_activation_after_controller_);
  param_loader.loadParam(
      yaml_prefix + "midair_activation/after_activation/tracker",
      _midair_activation_after_tracker_);

  param_loader.loadParam(yaml_prefix + "max_height_checking/enabled",
                         _max_height_enabled_);
  param_loader.loadParam(yaml_prefix + "max_height_checking/rate",
                         _max_height_checking_rate_);
  param_loader.loadParam(
      yaml_prefix + "max_height_checking/safety_height_offset",
      _max_height_offset_);
  ...
  param_loader.loadParam(yaml_prefix + "min_height_checking/rate",
                         _min_height_checking_rate_);
  param_loader.loadParam(
      yaml_prefix + "min_height_checking/safety_height_offset",
      _min_height_offset_);
  param_loader.loadParam(yaml_prefix + "min_height_checking/min_height",
                         _min_height_);

  param_loader.loadParam(yaml_prefix + "require_gain_manager",
                         _gain_manager_required_);
  param_loader.loadParam(yaml_prefix + "require_constraint_manager",
                         _constraint_manager_required_);

  param_loader.loadParam(yaml_prefix + "flight_timer/enabled",
                         _flighttime_timer_enabled_);
  param_loader.loadParam(yaml_prefix + "flight_timer/rate",
                         _flighttime_timer_rate_);
  param_loader.loadParam(yaml_prefix + "flight_timer/max_time",
                         _flighttime_max_time_);

  param_loader.loadParam(yaml_prefix + "max_throttle/enabled",
                         _maxthrottle_timer_enabled_);
  param_loader.loadParam(yaml_prefix + "max_throttle/rate",
                         _maxthrottle_timer_rate_);
  param_loader.loadParam(yaml_prefix + "max_throttle/max_throttle",
                         _maxthrottle_max_throttle_);
  param_loader.loadParam(yaml_prefix + "max_throttle/eland_timeout",
                         _maxthrottle_eland_timeout_);
  param_loader.loadParam(yaml_prefix + "max_throttle/ungrip_timeout",
                         _maxthrottle_ungrip_timeout_);

  param_loader.loadParam(yaml_prefix + "diagnostics/rate",
                         _diagnostics_timer_rate_);

  // | ------------------- scope timer logger ------------------- |

  param_loader.loadParam(yaml_prefix + "scope_timer/enabled",
                         scope_timer_enabled_);
  const std::string scope_timer_log_filename = param_loader.loadParam2(
      yaml_prefix + "scope_timer/log_filename", std::string(""));
  ...
  sh_controller_diagnostics_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::ControllerDiagnostics>(
          shopts, "~/controller_diagnostics_in");
  sh_odometry_ = mrs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(
      shopts, "~/odometry_in", &UavManager::callbackOdometry, this);
  sh_estimation_diagnostics_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::EstimationDiagnostics>(
          shopts, "~/odometry_diagnostics_in");
  sh_control_manager_diag_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::ControlManagerDiagnostics>(
          shopts, "~/control_manager_diagnostics_in");
  sh_mass_estimate_ = mrs_lib::SubscriberHandler<std_msgs::msg::Float64>(
      shopts, "~/mass_estimate_in");
  sh_throttle_ = mrs_lib::SubscriberHandler<std_msgs::msg::Float64>(
      shopts, "~/throttle_in");
  sh_height_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::Float64Stamped>(
      shopts, "~/height_in");
  sh_hw_api_status_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::HwApiStatus>(
      shopts, "~/hw_api_status_in");
  sh_gains_diag_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::GainManagerDiagnostics>(
          shopts, "~/gain_manager_diagnostics_in");
  sh_constraints_diag_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::ConstraintManagerDiagnostics>(
          shopts, "~/constraint_manager_diagnostics_in");
  sh_hw_api_gnss_ = mrs_lib::SubscriberHandler<sensor_msgs::msg::NavSatFix>(
      shopts, "~/hw_api_gnss_in", &UavManager::callbackHwApiGNSS, this);
  sh_max_height_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::Float64Stamped>(
  ...
  ph_diag_ = mrs_lib::PublisherHandler<mrs_msgs::msg::UavManagerDiagnostics>(
  ...
  ss_land_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/land_in",
      std::bind(&UavManager::callbackLand, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_land_home_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/land_home_in",
      std::bind(&UavManager::callbackLandHome, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_land_there_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::ReferenceStampedSrv>(
          node_, "~/land_there_in",
          std::bind(&UavManager::callbackLandThere, this, std::placeholders::_1,
                    std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_midair_activation_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/midair_activation_in",
      std::bind(&UavManager::callbackMidairActivation, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_min_height_check_ = mrs_lib::ServiceServerHandler<std_srvs::srv::SetBool>(
      node_, "~/enable_min_height_check_in",
      std::bind(&UavManager::callbackMinHeightCheck, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ...
  sch_ehover_ = mrs_lib::ServiceClientHandler<std_srvs::srv::Trigger>(
      node_, "~/ehover_out", cbkgrp_sc_);
  sch_switch_tracker_ = mrs_lib::ServiceClientHandler<mrs_msgs::srv::String>(
      node_, "~/switch_tracker_out", cbkgrp_sc_);
  sch_switch_controller_ = mrs_lib::ServiceClientHandler<mrs_msgs::srv::String>(
      node_, "~/switch_controller_out", cbkgrp_sc_);
  sch_emergency_reference_ =
      mrs_lib::ServiceClientHandler<mrs_msgs::srv::ReferenceStampedSrv>(
          node_, "~/emergency_reference_out", cbkgrp_sc_);
  sch_control_callbacks_ =
      mrs_lib::ServiceClientHandler<std_srvs::srv::SetBool>(
          node_, "~/enable_callbacks_out", cbkgrp_sc_);
  sch_arm_ = mrs_lib::ServiceClientHandler<std_srvs::srv::SetBool>(
      node_, "~/arm_out", cbkgrp_sc_);
  sch_odometry_callbacks_ =
      mrs_lib::ServiceClientHandler<std_srvs::srv::SetBool>(
  ...
  sch_toggle_control_output_ =
      mrs_lib::ServiceClientHandler<std_srvs::srv::SetBool>(
          node_, "~/toggle_control_output_out", cbkgrp_sc_);
  sch_offboard_ = mrs_lib::ServiceClientHandler<std_srvs::srv::Trigger>(
      node_, "~/offboard_out", cbkgrp_sc_);
...

  {
    std::function<void()> callback_fcn =
        std::bind(&UavManager::timerLanding, this);

    timer_landing_ = std::make_shared<TimerType>(
        timer_opts_no_start, rclcpp::Rate(_landing_timer_rate_), callback_fcn);
  }
  ...
  {
    std::function<void()> callback_fcn =
        std::bind(&UavManager::timerFlightTime, this);

    timer_flighttime_ = std::make_shared<TimerType>(
        timer_opts_no_start, rclcpp::Rate(_flighttime_timer_rate_),
        callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&UavManager::timerDiagnostics, this);

    timer_diagnostics_ = std::make_shared<TimerType>(
        timer_opts_start, rclcpp::Rate(_diagnostics_timer_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&UavManager::timerMidairActivation, this);

    timer_midair_activation_ = std::make_shared<TimerType>(
        timer_opts_no_start, rclcpp::Rate(_midair_activation_timer_rate_),
        callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&UavManager::timerMaxHeight, this);
    ...
    timer_max_height_ = std::make_shared<TimerType>(
        opts, rclcpp::Rate(_max_height_checking_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&UavManager::timerMinHeight, this);
    ...
    timer_min_height_ = std::make_shared<TimerType>(
        opts, rclcpp::Rate(_min_height_checking_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&UavManager::timerMaxthrottle, this);

    mrs_lib::TimerHandlerOptions opts;

    opts.node = node_;
    opts.autostart = hw_api_capabilities_.accepts_actuator_cmd ||
                     hw_api_capabilities_.accepts_control_group_cmd ||
                     hw_api_capabilities_.accepts_attitude_rate_cmd ||
                     hw_api_capabilities_.accepts_attitude_cmd;

    timer_maxthrottle_ = std::make_shared<TimerType>(
        opts, rclcpp::Rate(_maxthrottle_timer_rate_), callback_fcn);
  }
...
void UavManager::changeLandingState(LandingStates_t new_state) {
  ...
  switch (current_state_landing_) {
    case LANDING_STATE: {
      // we used to have a check here for freshness of the data.. this was
      // removed, since even old data on the mass estimate might be better than
      // the nominal mass from the parameter
  ...
  // just for ROS_INFO
  RCLCPP_INFO(node_->get_logger(), "Switching landing state %s -> %s",
...
void UavManager::timerHwApiCapabilities() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "UavManager::timerHwApiCapabilities",
...
void UavManager::timerLanding() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "UavManager::timerLanding",
                          scope_timer_logger_, scope_timer_enabled_);

  auto land_there_reference =
      mrs_lib::get_mutexed(mutex_land_there_reference_, land_there_reference_);
  ...
  if (sh_throttle_.hasMsg() &&
      (clock_->now() - sh_throttle_.lastMsgTime()).seconds() < 1.0) {
    desired_throttle = sh_throttle_.getMsg()->data;
  }

  auto res = transformer_->transformSingle(land_there_reference,
  ...
  if (res) {
    ...
    RCLCPP_ERROR(node_->get_logger(),
                 "could not transform the reference into the current frame! "
                 "land by yourself pls.");
    return;
  }
  ...
  if (current_state_landing_ == IDLE_STATE) {
    ...
    if (mrs_lib::geometry::dist(vec3_t(pos_x, pos_y, pos_z),
                                vec3_t(ref_x, ref_y, ref_z)) <
            _landing_tracking_tolerance_translation_ &&
        fabs(radians::diff(pos_heading, ref_heading)) <
            _landing_tracking_tolerance_heading_) {
      auto [success, message] = landWithDescendImpl();

      if (!success) {
        RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                              "call for landing failed: '%s'", message.c_str());
      }

    } else if (!control_manager_diagnostics->tracker_status.have_goal &&
               control_manager_diagnostics->flying_normally) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "the tracker does not have a goal while flying "
                           "home, setting the reference again");
...

      {
        ...
        auto response = transformer_->transformSingle(
            current_pose, land_there_reference_.header.frame_id);

        if (response) {
          land_there_reference_.reference.position.z =
              response.value().pose.position.z;
          RCLCPP_DEBUG(node_->get_logger(), "current altitude is %.2f m",
                       land_there_reference_.reference.position.z);

        } else {
          std::stringstream ss;
          ss << "could not transform current height to "
    ...
    // we should not attempt to finish the landing if some other tracked was
    // activated
    if (_landing_tracker_name_ ==
      ...
      if (desired_throttle) {
        ...
        throttle_mass_estimate_ =
            mrs_lib::quadratic_throttle_model::throttleToForce(
                _throttle_model_, desired_throttle.value()) /
            _g_;
        ...
        if (((throttle_mass_estimate_ <
              _landing_cutoff_mass_factor_ * landing_uav_mass_) ||
             desired_throttle < 0.01)) {
        ...
        if (z_vel > -0.1) {
          ...
          RCLCPP_INFO_THROTTLE(
              node_->get_logger(), *clock_, 500,
              "velocity over threshold for %.2f s",
...
void UavManager::timerTakeoff() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "UavManager::timerTakeoff",
                          scope_timer_logger_, scope_timer_enabled_);

  auto control_manager_diagnostics = sh_control_manager_diag_.getMsg();

  if (waiting_for_takeoff_) {
    if (control_manager_diagnostics->active_tracker == _takeoff_tracker_name_ &&
        control_manager_diagnostics->tracker_status.have_goal) {
      waiting_for_takeoff_ = false;
    } else {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "waiting for takeoff confirmation from the ControlManager");
      return;
    }
  }
  ...
  if (takingoff_) {
    if (control_manager_diagnostics->active_tracker != _takeoff_tracker_name_ ||
      ...
      // if there are any problems with this, it shoud be sufficient to only
      // overwrite the frame_id, without the position and heading here
      {
        std::scoped_lock lock(mutex_land_there_reference_);
      ...
      RCLCPP_INFO(node_->get_logger(), "take off finished, switching to %s",
                  _after_takeoff_tracker_name_.c_str());
...
void UavManager::timerMaxHeight() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "UavManager::timerMaxHeight",
                          scope_timer_logger_, scope_timer_enabled_);

  if (!sh_max_height_.hasMsg() || !sh_height_.hasMsg() ||
      !sh_control_manager_diag_.hasMsg() || !sh_odometry_.hasMsg()) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 10000,
                         "maxHeightTimer() not spinning, missing data");
    return;
  }
  ...
  auto res = transformer_->transformSingle(
      point, sh_height_.getMsg()->header.frame_id);
  ...
  if (res) {
    ...
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "timerMaxHeight() not working, cannot transform max z "
                         "to the height frame");
    return;
  }
  ...
  if (height > max_z_in_height) {
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "max height exceeded: %.2f >  %.2f, triggering safety goto", height,
        max_z_in_height);
    ...
    reference_out.reference.position.z =
        odometry_z + ((max_z_in_height - _max_height_offset_) - height);
...
void UavManager::timerMinHeight() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "UavManager::timerMinHeight",
                          scope_timer_logger_, scope_timer_enabled_);

  if (!sh_odometry_.hasMsg() || !sh_height_.hasMsg() ||
      !sh_control_manager_diag_.hasMsg() || !sh_odometry_.hasMsg()) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 10000,
                         "minHeightTimer() not spinning, missing data");
    return;
  }
  ...
  if (height < _min_height_) {
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "min height breached: %.2f < %.2f, triggering safety goto", height,
        _min_height_);
    ...
    reference_out.reference.position.y = odometry_y;
    reference_out.reference.position.z =
        odometry_z + ((_min_height_ + _min_height_offset_) - height);
...
void UavManager::timerFlightTime() {
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("timerFlightTime");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "UavManager::timerFlightTime",
                          scope_timer_logger_, scope_timer_enabled_);
...
void UavManager::timerMaxthrottle() {
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("timerMaxthrottle");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "UavManager::timerMaxthrottle",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (desired_throttle >= _maxthrottle_max_throttle_) {
    if (!maxthrottle_above_threshold_) {
      ...
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "max throttle exceeded threshold (%.2f/%.2f)",
                           desired_throttle, _maxthrottle_max_throttle_);

    } else {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                           "throttle over threshold (%.2f/%.2f) for %.2f s",
                           desired_throttle, _maxthrottle_max_throttle_,
  ...
  if (maxthrottle_above_threshold_ &&
      (clock_->now() - maxthrottle_first_time_).seconds() >
          _maxthrottle_ungrip_timeout_) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "throttle over threshold (%.2f/%.2f) for more than "
                         "%.2f s, ungripping payload",
                         desired_throttle, _maxthrottle_max_throttle_,
                         _maxthrottle_ungrip_timeout_);
  ...
  if (maxthrottle_above_threshold_ &&
    ...
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "throttle over threshold (%.2f/%.2f) for more than "
                          "%.2f s, calling for emergency landing",
                          desired_throttle, _maxthrottle_max_throttle_,
                          _maxthrottle_eland_timeout_);
...
void UavManager::timerDiagnostics() {
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("timerDiagnostics");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "UavManager::timerDiagnostics",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (sh_estimation_diagnostics_.hasMsg()) {  // get current position in lat-lon
    ...
    std::vector<std::string> state_estimators =
        estimation_diag.get()->switchable_state_estimators;

    got_gps_est = std::find(state_estimators.begin(), state_estimators.end(),
                            "gps_garmin") != state_estimators.end() ||
                  std::find(state_estimators.begin(), state_estimators.end(),
                            "gps_baro") != state_estimators.end();
    got_rtk_est = std::find(state_estimators.begin(), state_estimators.end(),
                            "rtk") != state_estimators.end();
  }
  ...
  if (takeoff_successful_) {
    if (got_gps_est || got_rtk_est) {
      auto land_there_reference = mrs_lib::get_mutexed(
          mutex_land_there_reference_, land_there_reference_);

      auto res =
          transformer_->transformSingle(land_there_reference, "latlon_origin");
...
void UavManager::timerMidairActivation() {
  ...
  if (sh_hw_api_status_.getMsg()->offboard) {
    ...
    {
      bool controller_switched =
          switchControllerSrv(_midair_activation_after_controller_);

      if (!controller_switched) {
        RCLCPP_ERROR(node_->get_logger(), "could not activate '%s'",
                     _midair_activation_after_controller_.c_str());
    ...
    {
      bool tracker_switched =
          switchTrackerSrv(_midair_activation_after_tracker_);

      if (!tracker_switched) {
        RCLCPP_ERROR(node_->get_logger(), "could not activate '%s'",
                     _midair_activation_after_tracker_.c_str());
  ...
  if ((clock_->now() - midair_activation_started_).seconds() > 0.5) {
    RCLCPP_ERROR(node_->get_logger(),
                 "waiting for OFFBOARD timeouted, reverting");
...
void UavManager::callbackHwApiGNSS(
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackHwApiGNSS");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "UavManager::callbackHwApiGNSS",
                          scope_timer_logger_, scope_timer_enabled_);
...
void UavManager::callbackOdometry(
...
bool UavManager::callbackTakeoff(
...

  {
...

    {
      if (!sh_control_manager_diag_.hasMsg() &&
          (clock_->now() - sh_control_manager_diag_.lastMsgTime()).seconds() >
              5.0) {
      ...
      if (_null_tracker_name_ !=
          sh_control_manager_diag_.getMsg()->active_tracker) {
        ss << "can not takeoff, need '" << _null_tracker_name_
           << "' to be active!";
        RCLCPP_ERROR_STREAM_THROTTLE(node_->get_logger(), *clock_, 1000,
                                     "" << ss.str());
        response->message = ss.str();
    ...
    if (_constraint_manager_required_ &&
        (!sh_constraints_diag_.hasMsg() ||
         (clock_->now() - sh_constraints_diag_.lastMsgTime()).seconds() >
             5.0)) {
    ...
    if (number_of_takeoffs_ > 0) {
      auto last_mass_difference = mrs_lib::get_mutexed(
          mutex_last_mass_difference_, last_mass_difference_);

      if (last_mass_difference > 1.0) {
        ss << std::setprecision(2);
        ss << "can not takeoff, estimated mass difference is too large: "
           << _null_tracker_name_ << "!";
        RCLCPP_ERROR_STREAM_THROTTLE(node_->get_logger(), *clock_, 1000,
                                     "" << ss.str());
        response->message = ss.str();
  ...
  // activating the takeoff controller
  {
    std::string old_controller =
        sh_control_manager_diag_.getMsg()->active_controller;
    ...
    if (!controller_switched) {
      std::stringstream ss;
      ss << "could not activate '" << _takeoff_controller_name_
         << "' for takeoff";
      RCLCPP_ERROR_STREAM_THROTTLE(node_->get_logger(), *clock_, 1000,
                                   "" << ss.str());
      response->success = false;
...
bool UavManager::callbackLandHome(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
bool UavManager::callbackLandThere(
  ...
  auto result =
      transformer_->transformSingle(reference_in, odometry->header.frame_id);
...
bool UavManager::callbackMidairActivation(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
std::tuple<bool, std::string> UavManager::landImpl(void) {
  // activating the landing controller
  {
    ...
    // Tomas: I pressume that its more important to get the UAV to the ground
    // rather than just throw out error.
    if (!controller_switched) {
      std::stringstream ss;
      ss << "could not activate '" << _takeoff_controller_name_
         << "' for landing";
  ...
  if (sh_height_.hasMsg()) {
    ...
    if (height > 0 && height >= _landing_descend_height_ + 1.0) {
...

      {
        ...
        // FOR FUTURE ME: Do not change this, we need it to be filled for the
        // final check later
        land_there_reference_.header.frame_id = "";
        land_there_reference_.header.stamp = clock_->now();
        land_there_reference_.reference.position.x =
            odometry->pose.pose.position.x;
        land_there_reference_.reference.position.y =
            odometry->pose.pose.position.y;
        land_there_reference_.reference.position.z =
            odometry->pose.pose.position.z -
            (height - _landing_descend_height_);
        land_there_reference_.reference.heading =
            mrs_lib::AttitudeConverter(odometry->pose.pose.orientation)
                .getHeading();
      }
...
std::tuple<bool, std::string> UavManager::midairActivationImpl(void) {
  ...
  std::string old_controller;
  {
    ...
    bool controller_switched =
        switchControllerSrv(_midair_activation_during_controller_);

    if (!controller_switched) {
      std::stringstream ss;
      ss << "could not activate '" << _midair_activation_during_controller_
         << "' for midair activation";
      RCLCPP_ERROR_STREAM_THROTTLE(node_->get_logger(), *clock_, 1000,
                                   "" << ss.str());
  ...
  // this will cause the Control Manager to output something else than
  // min-throttle
  std::string old_tracker;
  {
    ...
    bool tracker_switched =
        switchTrackerSrv(_midair_activation_during_tracker_);
    ...
    if (!tracker_switched) {
      ...
      std::stringstream ss;
      ss << "could not activate '" << _midair_activation_during_tracker_
         << "' for midair activation";
      RCLCPP_ERROR_STREAM_THROTTLE(node_->get_logger(), *clock_, 1000,
                                   "" << ss.str());
  ...
  // 4. wait for 100 ms, that should be enough for the Pixhawk to start getting
  // data
  ...
  // start the timer which should check if the offboard is on, activate proper
  // controller and tracker or timeout
...
void UavManager::setControlCallbacksSrv(const bool& input) {
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_WARN(node_->get_logger(),
                  "service call for setting control callbacks returned: %s.",
                  response.value()->message.c_str());
    }

  } else {
    RCLCPP_ERROR(node_->get_logger(),
                 "service call for setting control callbacks failed!");
  }
}
...
bool UavManager::toggleControlOutput(const bool& input) {
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "service call for control output returned: %s.",
                           response.value()->message.c_str());
      return false;
    } else {
      return true;
    ...
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "service call for control output failed!");
    return false;
  }
}
...
bool UavManager::offboardSrv(const bool in) {
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "service call for offboard failed, returned: %s",
                            response.value()->message.c_str());
      return false;
    } else {
      return true;
    ...
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "service call for offboard failed!");
    return false;
  }
}
...
void UavManager::disarmSrv(void) {
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "service call disarming returned: %s.",
                           response.value()->message.c_str());
    }
...
bool UavManager::switchControllerSrv(const std::string& controller) {
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_WARN(node_->get_logger(),
                  "service call for switching controller returned: '%s'",
                  response.value()->message.c_str());
    }
    ...
    RCLCPP_ERROR(node_->get_logger(),
                 "service call for switching controller failed!");
...
bool UavManager::switchTrackerSrv(const std::string& tracker) {
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_WARN(node_->get_logger(),
                  "service call for switching tracker returned: '%s'",
                  response.value()->message.c_str());
    }
    ...
    RCLCPP_ERROR(node_->get_logger(),
                 "service call for switching tracker failed!");
...
bool UavManager::landSrv(void) {
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_WARN(node_->get_logger(),
                  "service call for landing returned: '%s'",
                  response.value()->message.c_str());
    }
...
bool UavManager::ehoverSrv(void) {
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_WARN(node_->get_logger(), "service call for ehover returned: '%s'",
                  response.value()->message.c_str());
    }
...
bool UavManager::emergencyReferenceSrv(
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "service call for emergency reference returned: '%s'",
          response.value()->message.c_str());
    }
    ...
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "service call for emergency reference failed!");
