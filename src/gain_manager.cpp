...
class GainManager : public mrs_lib::Node {
 ...
 private:
  ...
  mrs_lib::ServiceClientHandler<rcl_interfaces::srv::SetParameters>
      sc_set_gains_;
  ...
  //
  mrs_lib::PublisherHandler<mrs_msgs::msg::GainManagerDiagnostics>
      ph_diagnostics_;
...
GainManager::GainManager(rclcpp::NodeOptions options)
...
void GainManager::initialize() {
  ...
  for (it = _gain_names_.begin(); it != _gain_names_.end(); ++it) {
    ...
    param_loader.loadParam(yaml_prefix + *it + "/horizontal/kp",
                           new_gains.kpxy);
    param_loader.loadParam(yaml_prefix + *it + "/horizontal/kv",
                           new_gains.kvxy);
    param_loader.loadParam(yaml_prefix + *it + "/horizontal/ka",
                           new_gains.kaxy);
    param_loader.loadParam(yaml_prefix + *it + "/horizontal/kib",
                           new_gains.kibxy);
    param_loader.loadParam(yaml_prefix + *it + "/horizontal/kiw",
                           new_gains.kiwxy);
    param_loader.loadParam(yaml_prefix + *it + "/horizontal/kib_lim",
                           new_gains.kibxy_lim);
    param_loader.loadParam(yaml_prefix + *it + "/horizontal/kiw_lim",
                           new_gains.kiwxy_lim);
    ...
    param_loader.loadParam(yaml_prefix + *it + "/attitude/kq_roll_pitch",
                           new_gains.kqrp);
    param_loader.loadParam(yaml_prefix + *it + "/attitude/kq_yaw",
                           new_gains.kqy);

    param_loader.loadParam(yaml_prefix + *it + "/mass_estimator/km",
                           new_gains.km);
    param_loader.loadParam(yaml_prefix + *it + "/mass_estimator/km_lim",
                           new_gains.km_lim);
  ...
  for (it = _current_state_estimators_.begin();
       it != _current_state_estimators_.end(); ++it) {
    ...
    for (it2 = temp_vector.begin(); it2 != temp_vector.end(); ++it2) {
      if (!stringInVector(*it2, _gain_names_)) {
        RCLCPP_ERROR(
            node_->get_logger(),
            "the element '%s' of %s/allowed_gains is not a valid gain!",
            it2->c_str(), it->c_str());
        rclcpp::shutdown();
        exit(1);
      }
    ...
    _map_type_allowed_gains_.insert(
  ...
  for (it = _current_state_estimators_.begin();
    ...
    _map_type_default_gains_.insert(
  ...
  ss_set_gains_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::String>(
      node_, "~/set_gains_in",
      std::bind(&GainManager::callbackSetGains, this, std::placeholders::_1,
                std::placeholders::_2),
...

  {
    std::function<void()> callback_fcn =
        std::bind(&GainManager::timerGainManagement, this);

    timer_gain_management_ = std::make_shared<TimerType>(
        timer_opts_start, rclcpp::Rate(_gain_management_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&GainManager::timerDiagnostics, this);
...
bool GainManager::setGains(std::string gains_name) {
  ...
  if (it == _gains_.end()) {
    RCLCPP_WARN(node_->get_logger(),
                "can not set gains for '%s', the mode is not on a list!",
                gains_name.c_str());
    return false;
  }
  ...
  RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                       "setting up gains for '%s'", gains_name.c_str());

  auto response = sc_set_gains_.callSync(request);

  if (!response) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "the service for setting gains has failed!");
    return false;

  } else {
    for (auto res : response.value()->results) {
      if (!res.successful) {
        RCLCPP_ERROR(node_->get_logger(), "could not set param: %s",
                     res.reason.c_str());
      }
    }
...
bool GainManager::callbackSetGains(
    const std::shared_ptr<mrs_msgs::srv::String::Request> request,
    const std::shared_ptr<mrs_msgs::srv::String::Response> response) {
  if (!is_initialized_) {
    return false;
  ...
  if (!stringInVector(request->value, _gain_names_)) {
    ss << "the gains '" << request->value.c_str()
       << "' do not exist (in the GainManager's config)";
  ...
  if (!stringInVector(request->value,
                      _map_type_allowed_gains_.at(
                          estimation_diagnostics->current_state_estimator))) {
    ss << "the gains '" << request->value.c_str()
       << "' are not allowed given the current state estimator";
...
void GainManager::timerGainManagement() {
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("gainManagementTimer");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "GainManager::gainManagementTimer",
  ...
  auto current_gains =
      mrs_lib::get_mutexed(mutex_current_gains_, current_gains_);
  ...
  if (estimation_diagnostics->current_state_estimator != last_estimator_name) {
    ...
    it = _map_type_default_gains_.find(
        estimation_diagnostics->current_state_estimator);

    if (it == _map_type_default_gains_.end()) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "the state estimator '%s' was not specified in the gain_manager's "
          "config!",
          estimation_diagnostics->current_state_estimator.c_str());
      ...
      if (stringInVector(
        ...
        RCLCPP_WARN_THROTTLE(
            node_->get_logger(), *clock_, 1000,
            "the current gains '%s' are not within the allowed gains for '%s'",
            current_gains.c_str(),
            estimation_diagnostics->current_state_estimator.c_str());
...
void GainManager::timerDiagnostics() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "GainManager::timerDiagnostics",
                          scope_timer_logger_, scope_timer_enabled_);

  if (!sh_estimation_diag_.hasMsg()) {
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 10000,
        "can not do gain management, missing estimator diagnostics!");
    return;
  }
  ...
  if (!sh_control_manager_diag_.hasMsg()) {
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 10000,
        "can not do gain management, missing control manager diagnostics!");
    return;
  }
  ...
  if (current_gains ==
      "") {  // this could happend just before timerGainManagement() finishes
    return;
  }
  ...
  // get the available gains
  {
    ...
    it = _map_type_allowed_gains_.find(
        estimation_diagnostics->current_state_estimator);

    if (it == _map_type_allowed_gains_.end()) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "the estimator name '%s' was not specified in the gain_manager's "
          "config!",
          estimation_diagnostics->current_state_estimator.c_str());
    } else {
  ...
  // get the current gain values
  {
    ...
    if (it == _gains_.end()) {
      RCLCPP_ERROR(node_->get_logger(),
                   "current gains '%s' not found in the gain list!",
                   current_gains.c_str());
      return;
    }
...
bool GainManager::stringInVector(const std::string& value,
                                 const std::vector<std::string>& vector) {
